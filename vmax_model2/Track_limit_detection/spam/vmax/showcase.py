...
def main():
    ...
    data=r.tb.annotate_document(data)
    rows=data['frames']
    ...
    for i in indices:
        ...
        status={'outside':'ALL FOUR TREADS OUT','within':'WITHIN TRACK LIMITS','review':'BOUNDARY REVIEW'}[row['boundary_status']]
        d.text((50,r.HEIGHT-64),status,font=small,fill=(255,82,71) if row['is_violation'] else (75,222,173))
        d.text((r.WIDTH-265,35),f'TREAD MARGIN >= {row["boundary_margin_m_interval"][0]:+.3f} m',font=small,fill='white')
