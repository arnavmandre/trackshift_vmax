"""Validate rendered outputs, legacy centre labels and revised tyre-boundary labels."""
...
def independent(rows):
    worst=0.
    for row in rows:
        legacy=row.get('legacy_center_rule',row)
        ...
        error=float(abs(dist-np.array(list(legacy['corner_excess_m'].values()))).max());worst=max(worst,error)
        assert error<1e-9 and bool(dist.min()>0)==legacy['is_violation']
        if 'tyre_boundaries' in row:
            checked=r.tb.annotate_row(row,row.get('camera'))
            assert checked['is_violation'] is row['is_violation']
            np.testing.assert_allclose(checked['boundary_margin_m_interval'],row['boundary_margin_m_interval'],atol=1e-12)
    return worst

def main():
    report={'fixed_camera_clips':24,'fixed_camera_resolution':[1280,720],'legacy_center_labels_preserved':True,'label_schema':'vmax.tyre_boundaries.v1','distance_error_m':{},'scenarios':{}}
    for source in sorted((r.ROOT/'data').glob('*/ground_truth.json')):
        ...
        report['distance_error_m'][name]=independent(truth['frames']);report['scenarios'][name]={'legacy_center_events':truth['events']}
        for cam in r.CAMS:
            enriched=json.loads((folder/(cam['name']+'.json')).read_text())
            for expected,actual in zip(truth['frames'],enriched['frames']):
                assert all((actual['legacy_center_rule'][k] if k in actual['legacy_center_rule'] else actual[k])==v for k,v in expected.items())
                check=r.tb.annotate_row(actual,cam)
                assert check['tyre_boundaries']==actual['tyre_boundaries']
                assert check['is_violation'] is actual['is_violation']
    ...
    assert abs(lat).max()<3 and abs(max(row['legacy_center_rule']['min_excess_m'] for row in rows)-.15)<1e-8
    report['race_pace']={'speed_range_kmh':(np.array([speed.min(),speed.max()])*3.6).tolist(),'peak_lateral_g':float(abs(lat).max()),'legacy_center_peak_margin_m':max(row['legacy_center_rule']['min_excess_m'] for row in rows),'events':showcase['events'],'opengl_projection_error_px':showcase['opengl_projection_error_px']}
    report['cameras']=r.calibrate()
    report['exported_car']=export_car()
    (r.OUT/'validation.json').write_text(json.dumps(report,indent=2))
    lines=['# VMAX v2 validation','', 'All 25 clips passed frame-count, resolution and fps checks. Legacy centre-rule fields are retained separately. New decisions use the complete ground-tread boundary.','', '| Check | Result |','|---|---|',f"| Independent tyre-distance agreement | {max(report['distance_error_m'].values()):.3g} m maximum error |",f"| Race-pace speed range | {speed.min()*3.6:.2f}–{speed.max()*3.6:.2f} km/h |",f"| Race-pace peak lateral acceleration | {abs(lat).max():.3f} g |",'| Race-pace legacy centre peak margin | 0.150000 m |',f"| Race-pace camera matrix agreement | {showcase['opengl_projection_error_px']:.3g} pixels |",'', '| Camera | Curved-boundary coverage | Matrix agreement (px) | Float32 in-frame error (px) |','|---|---:|---:|---:|']
