> Historical v2 centre-label report. The boundary-labelled export is in
> `../../data_boundaries/`; see its `boundary_export_report.json` and `README.md`.
> Eight boundary geometry tests pass via `python -m unittest discover -s spam/vmax -p 'test_*.py'`
> from the trackshift root. The revised OpenGL render path has not been rerun in this environment.
