...
def debug(im,cam,rows):
    from export_boundaries import draw_debug
    return draw_debug(im,[tb.annotate_row(row,cam) for row in rows],cam)
...
def main():
    ...
    for source in sources:
        ...
        for cam in CAMS:
            ...
            enriched=tb.annotate_document(data,cam);enriched['render_version']='VMAX v2 / boundary labels v1';(folder/(cam['name']+'.json')).write_text(json.dumps(enriched,indent=2))
            print(name,cam['name'],'rendered',flush=True)
        report['scenarios'][name]=enriched['events']
